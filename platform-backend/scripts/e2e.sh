...
export COMPUTE_BASE_URL="http://compute-stub:8000"
export COMPUTE_SERVICE_KEY="test-compute-key"
...
export PAYMENT_RECONCILE_PENDING_SECONDS=1
export PAYMENT_RECONCILE_SWEEP_SECONDS=5
...
cd "$ROOT_DIR"
uv run pytest -q \
  tests/e2e/test_mvp_happy_path.py \
  tests/e2e/test_failure_recovery.py \
  tests/e2e/test_security_boundaries.py \
  tests/e2e/test_api_contract_matrix.py
