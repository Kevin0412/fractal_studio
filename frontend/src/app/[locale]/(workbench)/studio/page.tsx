...
import { ChevronDown, Heart, Save, Sparkles, WandSparkles, X } from "lucide-react";
...
export default function StudioPage() {
  ...
  const zoom = (factor: number) => update({ scale: Math.min(1e9, Math.max(1e-12, Number(spec.scale ?? 3) * factor)) });
  ...
  return <div className="mx-auto max-w-[1500px] space-y-5">
    <header className="flex flex-wrap items-end justify-between gap-3"><div><p className="mb-1 text-xs font-medium uppercase tracking-[0.18em] text-fractal-300">Create a fractal</p><h1 className="text-3xl font-semibold">Explore, don’t configure.</h1><p className="mt-1 text-sm text-muted-foreground">Pick a look, zoom into details, drag the image around. Everything updates by itself.</p></div><div className="flex gap-2"><Button variant="outline" onClick={saveView}><Heart className="mr-2 h-4 w-4" />Save this view</Button><Button onClick={() => void saveAndRender()}><Save className="mr-2 h-4 w-4" />Make PNG</Button></div></header>
    <div className="grid gap-5 xl:grid-cols-[20rem_minmax(0,1fr)]">
      <aside className="space-y-4 rounded-2xl border border-white/10 bg-white/[0.025] p-4 xl:sticky xl:top-4 xl:self-start">
        ...
        <section className="rounded-xl border border-white/10 bg-black/15 p-3 text-sm"><p className="font-medium">3 · Explore canvas</p><p className="mt-1 text-muted-foreground">Scroll or use <b>+ / −</b> to zoom. Drag image to move. Reset returns to home.</p></section>
