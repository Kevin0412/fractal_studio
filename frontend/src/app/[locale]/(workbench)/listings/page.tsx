...
export default function ListingsPage() {
  ...
  return <div className="space-y-5"><div><h1 className="text-2xl font-semibold">My listings</h1><p className="text-muted-foreground">Draft from Library, edit offer, then publish.</p></div>{error && <p className="text-red-400">{error}</p>}<div className="space-y-3">{listings.map((listing) => <article key={listing.id} className="rounded-xl border border-white/10 p-4"><div className="flex flex-wrap items-center justify-between gap-2"><div><b>{listing.title}</b><p className="text-sm text-muted-foreground">{listing.price} CNY · {listing.status}</p></div><div className="flex gap-2"><Button size="sm" variant="outline" onClick={() => edit(listing)}>Edit title</Button><Button size="sm" disabled={listing.status !== "published" && !listing.preview?.watermarkedPreviewUrl} onClick={() => change(listing)}>{listing.status === "published" ? "Unpublish" : "Publish"}</Button></div></div><p className="mt-2 text-sm">{listing.description || "No description"}</p>{listing.status !== "published" && !listing.preview?.watermarkedPreviewUrl && <p className="mt-2 text-xs text-muted-foreground">Public preview is still being prepared.</p>}</article>)}</div></div>;
}
