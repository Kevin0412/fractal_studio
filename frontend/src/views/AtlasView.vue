<template>
  <div>
    <RunConfigForm title="Atlas" description="Run all variants including ln/e^x mapping" @run="run" />
    <JobStatusPanel :status="status" :run-id="runId" />
    <section class="view-card" v-if="imageUrl">
      <h3>Latest Atlas Image</h3>
      <img :src="imageUrl" alt="atlas" class="preview-image" />
    </section>
  </div>
</template>
...
const run = async () => {
  ...
  try {
    const res = await invokeModule('atlas')
